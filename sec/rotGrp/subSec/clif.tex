\subsection{Clifordo algebros}


Toliau nagrinėsime nagrinėsime dvi Clifordo algebras $\mathup{Cl}^+(3)$ ir
$\mathup{Cl}^-(3)$. Tai yra vectorinės erdvės su papildoma bitiesine daugybos
operacija, kuri leidžia sudauginti du algebros vektorius ir gauti naują
vektorių algebroje. Šias algebras apibrėšime pirma paimdami daug bendresnę
tenzorinę algebrą ir tada jai sutekdami norimą struktūrą.

\begin{definition}
	Vienetinė tenzorinė algebra $T(\set R^3)$ yra tiesioginė suma
	\begin{equation}
		T(\mathbb{R}^3) 
		=
		\bigoplus_{k=0}^{\infty} T^k
		=
		\mathbb{R} 
		\oplus \set R^3 
		\oplus (\set R^3 \otimes \set R^3) 
		\oplus (\set R^3 \otimes \set R^3 \otimes \set R^3) 
		\oplus \dots
		\,,
	\end{equation}
	$T^k$ čia yra vektorinės erdvės $T^0 = \set R$\: ,\: $T^1=\set R^3$\: ,\: $T^2=\set R^3 \otimes \set R^3$\: ,\:
	$T^3=\set R^3 \otimes \set R^3 \otimes \set R^3$
	ir taip toliau. Algebros daugyba yra duodama vektorių tenzorinės sandaugos.
	Vienetinis tenzorinės algebros elementas yra žymimas $\mathbb 1$\:.
\end{definition}
Kitaip paskius tai yra algebra, kurios vektoriai yra ivairių ilgiu tenzorinių
sandaugos $\vb v_1 \otimes \vb v_2 \otimes \dots \otimes \vb v_n$\:,\: $\vb v_i \in \set R^3$
ir tokių sandaugų sumos.
\begin{example}
	Vektorius $3\cdot\mathbb 1 + \vb v$ priklauso $T(\set R^3)$ ir yra suma vektorių iš $T^0$ ir $T^1$.
	Vektorius $2\vb w + \vb v \otimes \vb w$ yra suma vektorių iš $T^1$ ir $T^2$.
	Jų sandauga tenzorinėje algebroje yra
	\begin{equation}
		\begin{split}
			\qty(3\cdot\mathbb 1 + \vb v)
			\otimes
			\qty(2\vb w + \vb v \otimes \vb w)
			=&
			3\cdot\mathbb 1 \otimes 2\vb w
			+ 3\cdot\mathbb 1 \otimes \vb v \otimes \vb w
			+ \vb v \otimes 2\vb w
			+ \vb v \otimes \vb v \otimes \vb w
			\\=&
			3\cdot2\qty(\mathbb 1 \otimes \vb w)
			+ 3\qty(\mathbb 1 \otimes \vb v \otimes \vb w)
			+ 2(\vb v \otimes \vb w)
			+ \qty(\vb v \otimes \vb v \otimes \vb w)
			\\=&
			6 \vb w
			+ 3\qty(\vb v \otimes \vb w)
			+ 2(\vb v \otimes \vb w)
			+ \qty(\vb v \otimes \vb v \otimes \vb w)
			\\=&
			6 \vb w
			+ 5\qty(\vb v \otimes \vb w)
			+ \qty(\vb v \otimes \vb v \otimes \vb w)
			\,,
		\end{split}
	\end{equation}
	čia pirmoje eilutėje pasinaudojome tuo kad tenzorinės sandauga
	distributyvi; antroje eilutėje naudojames tuo kad tenzorinė sandauga
	tiesiška kiekvieno dauginamo vektoriaus atžvilgiu, kad iškeltųme 
	skaliarinius daugiklius; trečioje eilutėje sudedame tiesiškai
	priklausomus vektorius.
	Gautas vektorius yra suma vektorių iš $T^1$, $T^2$ ir $T^3$.
\end{example}


Dabar $T(\set R^3)$ suteiksime papildomą strutūrą, kad algebroje
$\mathup{Cl}^+(3)$ būtų tenkinamas sąryšis
\begin{equation}
	\vb v \otimes \vb w
	+
	\vb w \otimes \vb v
	=
	2(\vb v , \vb w) \mathbb 1
	\quad ,
	\label{eq:cl+}
\end{equation}
o algebroje $\mathup{Cl}^-(3)$ tenkinamas sąryšis
\begin{equation}
	\vb v \otimes \vb w
	+
	\vb w \otimes \vb v
	=
	- 2(\vb v , \vb w) \mathbb 1
	\quad .
	\label{eq:cl-}
\end{equation}
Šiuos sąryšius ,,užkoduojame'' į aibes $S^\pm \subset T(\set R^3)$
\begin{equation}
	\begin{split}
		S^+ = \qty{
			\vb v \otimes \vb w
			+
			\vb w \otimes \vb v
			-
			2(\vb v , \vb w) \mathbb 1
			\mid
			\vb v, \vb w \in \set R^3
		}
		\\
		S^- = \qty{
			\vb v \otimes \vb w
			+
			\vb w \otimes \vb v
			+
			2(\vb v , \vb w) \mathbb 1
			\mid
			\vb v, \vb w \in \set R^3
		}\,.
	\end{split}
\end{equation}
Užrašome tenzorinės algebros poalgebrius $I^\pm \subset T(\set R^3)$,
gauamus iš visų vektorių, kurie turi bent vieną daugiklį iš $S^\pm$
\begin{equation}
	I^\pm = \qty{
		\sum_{\substack{
			s \in S^\pm \\ 
			a,b \in \mathup{T(V)} 
		}}
		a \otimes s \otimes b
	}
	\quad .
\end{equation}
Šie poalbebriai papildomai tenkina savybę, kad betkokiem $a \in T(\set R^3)$
ir $d \in I^\pm$, $a\otimes d, d \otimes a \in I^\pm$. Poalgebris
tenkinantis šia savybę yra vadinamas algebros idealu.
Literatūroje šią konstrukciją yra įprasta pateikti sakiniu:
,,$I^\pm$ yra algebros $T(\set R^3)$ idealas genaruojams 
sąryšio (\ref{eq:cl+}) ar atitinkamai (\ref{eq:cl-})''.

Duotam vektoriui $a \in T(\set R^3)$ aibė
$\qty{a+d \mid d \in I^\pm}$ (dar žymima $a+I^\pm$)
vadinama šonine klase, o vektorius $a$ vadinamas klasės atstovu.
Dviejų šoninių klasių $a+I^\pm$ ir $b + I^\pm$ sandauga yra šoninė
klasė, kuriai priklauso atstovų sandauga algebroje $a \otimes b$.
Ši sandauga nepriklauso nuo pasirinktų klasės atstovų, nes
\begin{equation}
	(a+I^\pm)\otimes(b+I^\pm)
	=
	a \otimes b
	+ a \otimes I^\pm
	+ I^\pm \otimes b
	+ I^\pm \otimes I^\pm
	=
	a \otimes b + I^\pm
	\,.
\end{equation}
Sumos nariai $
a \otimes I^\pm
= I^\pm \otimes b
= I^\pm \otimes I^\pm
= I^\pm
$, nes $I^\pm$ yra algebros $T(\set R^3)$ idealas.
Tai tikrai
\begin{equation}
	(a+I^\pm)\otimes(b+I^\pm)
	=
	a \otimes b + I^\pm
	\,.
\end{equation}



\begin{definition}
	Algebra $\mathup{Cl}^\pm(3)$ yra faktorinė algebra $T(\set R^3)/I^\pm$,
	kurios elementai yra šoninės klasės $a+I^\pm$, $a \in T(\set R^3)$
	ir jų sandauga yra $(a+I^\pm)\otimes(b+I^\pm)=a\otimes b + I^\pm.$
\end{definition}

\begin{example}
	$\vb w \otimes \vb v +\vb v \otimes \vb w + I^+$ 
	yra šoninė klasė,
	kuri yra vektorius algebroje $\mathup{Cl}^+(3)$. 
	Suparastiname išraišką
	\begin{equation}
		\vb w \otimes \vb v +\vb v \otimes \vb w + I^+
		= 
		\vb w \otimes \vb v +\vb v \otimes \vb w 
		- \qty(
			\vb v \otimes \vb w + \vb w \otimes \vb v 
			- 2(\vb v,\vb w)\mathbb 1
		) 
		+ I^+
		=
		2(\vb v,\vb w)\mathbb 1 + I^+
		\,,
	\end{equation}
	čia pasinaudojome tuo kad 
	$\qty( \vb v \otimes \vb w + \vb w \otimes \vb v - 2(\vb v,\vb w)\mathbb 1) \in I^+$.
	Pavyko parodyti, kad $\vb w \otimes \vb v + \vb v \otimes \vb w$ 
	ir $2(\vb v,\vb w) \mathbb 1$ atstovauja tą pačią klasę.
\end{example}

\begin{example}
	Tegul $a,b \in T(\set R^3)$ ir
	$
		a \otimes (\vb v \otimes \vb w + 2(\vb v,\vb w)\mathbb 1) \otimes b + I^-
	$
	yra šoninė klasė, kuri yra vektorius iš $\mathup{Cl}^-(3)$.
	Šią išraišką supapranstiname atlidami veiksmus
	\begin{equation}
		\begin{split}
			a \otimes (\vb v \otimes \vb w + 2(\vb v,\vb w)\mathbb 1) \otimes b + I^-
			=&
			a \otimes (\vb v \otimes \vb w + 2(\vb v,\vb w)\mathbb 1) \otimes b 
			- a \otimes \qty(\vb v \otimes \vb w + \vb w \otimes \vb v +2(\vb v,\vb w) \mathbb 1)  \otimes b 
			+ I^-
			\\=&
			a \otimes \qty(
				\vb v \otimes \vb w + 2(\vb v,\vb w)\mathbb 1
				- \vb v \otimes \vb w - \vb w \otimes \vb v
				-2(\vb v,\vb w) \mathbb 1) 
			)\otimes b 
			+ I^-
			\\=&
			a \otimes \qty(
				- \vb w \otimes \vb v 
			)\otimes b 
			+ I^-
			\\=&
			- a \otimes 
				\vb w \otimes \vb v 
			\otimes b 
			+ I^-
			\,.
		\end{split}
	\end{equation}
	Parodėme, kad sudarius faktorinę algebrą $\mathup{Cl}^-(3)$,
	galime jos viduje taikyti sąryšį (\ref{eq:cl-})
	manipuliuojant šoninės klasės atstovus.
\end{example}
Toliau priimame sutrumpintą žymėjimą, nes išreikštai
su šoninėmis klasėmis nebedirbsime. Užrašysime tik
klasės atstovą, o kad išraiška skirtūsi nuo 
vektorių tenzorinės sandaugos vietoje ženklo ,,$\otimes$''
tiesiog rašysime vektorius vieną šalia kito.
Vietoje
\begin{equation}
	a \otimes (\vb v \otimes \vb w + \vb v \otimes \vb w) \otimes b + I^-
	=
	-2(\vb v,\vb w) \qty(a \otimes b) + I^-
\end{equation}
rašysime
\begin{equation}
	a (\vb v \vb w + \vb v \vb w) b
	=
	-2(\vb v,\vb w) a b
	\,.
\end{equation}

Štai kelios svarbios tapatybės
\begin{preposition}
	Algebrose $\mathup{Cl}^\pm(3)$ 
	\begin{equation}
		\vb v \vb v 
		= \frac{1}{2}(\vb v \vb v + \vb v \vb v) 
		= \pm(\vb v,\vb v) \mathbb 1
		\,.
	\end{equation}
\end{preposition}

\begin{preposition}
	\label{prep:R_action-}
	Algebroje $\mathup{Cl}^+(3)$ 
	\begin{equation}
		-\vb v \vb w \vb v 
		= -\qty( 2(\vb v,\vb w)\mathbb 1 - \vb{wv} )\vb v 
		= -2(\vb v,\vb w) \vb v + \vb{wvv} 
		= -2(\vb v,\vb w) \vb v + (\vb v,\vb v)\vb w 
		\,.
	\end{equation}
\end{preposition}

\begin{preposition}
	\label{prep:R_action+}
	Algebroje $\mathup{Cl}^-(3)$ 
	\begin{equation}
		\vb v \vb w \vb v 
		= \qty( - 2(\vb v,\vb w)\mathbb 1 - \vb{wv} )\vb v 
		= -2(\vb v,\vb w) \vb v - \vb{wvv} 
		= -2(\vb v,\vb w) \vb v + (\vb v,\vb v)\vb w 
		\,.
	\end{equation}
\end{preposition}

Atkreipiame dėmesį, kad kai $(\vb v,\vb v)=1$,
$(\vb v,\vb w)\vb v$ yra vektoriaus $\vb w$ projekcija į $\vb v$
ir $-2(\vb v,\vb w)\vb v + \vb w$ yra vektoriaus atspindys
per plokštumą statmeną vektoriui $\vb v$. 

Tegul $\qty{\vb e_1,\vb e_2,\vb e_3}$ yra ortonormali $\set R^3$ bazė.
Bazinius vektorius galime perketi į algebra $\mathup{Cl^\pm(V)}$,
kur jie turi tenkinti
\begin{equation}
	\begin{split}
		\vb e_i \vb e_j &= - \vb e_j \vb e_i \ , \ i \neq j \\
		\vb e_i \vb e_i &= \pm \mathbb 1
		\quad .
	\end{split}
	\label{eq:base-relations}
\end{equation}
Su šių sąryšių pagalba galime bet kokį $\mathup{Cl^\pm(V)}$ elementą užrašyti,
kaip tiesinę kombinaciją aštuonių elementų:
\begin{equation}
		\mathbb 1 ,\,
		\vb e_1 ,\, \vb e_2 ,\, \vb e_3 ,\, 
		\vb e_2 \vb e_3 ,\, \vb e_3 \vb e_1 ,\, \vb e_1 \vb e_2 ,\,
		\vb e_1 \vb e_2 \vb e_3 ,
\end{equation}
nes su pirma taisykle betkurį tenzorinės algebros bazės elementą galima išrikiuoti taip kad
ortų indeksai būtų surašyti didėjimo tvarka ir su antra taisykle pasikartojantys ortai panaikinami.
Tai reiškia $\mathup{Cl^\pm(V)}$ yra aštuonmatės algebros.


\begin{definition}
	Transponuota $\mathup{Cl}^\pm(3)$ bazė yra 
	\begin{equation}
		\qty(\vb e_{i_1} \vb e_{i_2} \dots \vb e_{i_k} )^t
		:=
		\vb e_{i_k} \vb e_{i_{k-1}} \dots \vb e_{i_1}
		=
		(-1)^{k-1} \vb e_{i_1} \vb e_{i_2} \dots \vb e_{i_k} 
	\end{equation}
	ir transponavimo operacija yra tiesiškai praplečiama
	bet kuriam algebros $\mathup{Cl}^\pm(3)$ vektoriui, veikimu ant bazės.
	Transponavimas yra algebros anti-homomorfizmas,
	tai reiškia, kad betkuriems algebros elementams $a$ ir $b$ galioja
	\begin{equation}
		(a b)^t = b^t a^t
		\,.
	\end{equation}
\end{definition}


\begin{definition}
	Apibrėžiame algebros homomorfizmo $\alpha$ veikimą ant bazės
	\begin{equation}
		\alpha( \vb e_{i_1} \vb e_{i_2} \dots \vb e_{i_k} )
		:=
		(-1)^{k} \vb e_{i_1} \vb e_{i_2} \dots \vb e_{i_k} 
		\,.
	\end{equation}
	Tiesiškai $\alpha$ veikimą išplėčiame ant visos algebros $\mathup{Cl}^\pm(3)$,
	homomorfizmo veikimu ant bazės.
\end{definition}

 Pažymime algebros poerdvius
\begin{equation}
	\begin{split}
		A_0 &:= \qty{
			v \in \mathup{Cl}^\pm(3) \mid \alpha(v)=v
		}
		\,,
		\\
		A_1 &:= \qty{
			v \in \mathup{Cl}^\pm(3) \mid \alpha(v)=-v
		}
		\,.
	\end{split}
\end{equation}
Susiejant su erdvės baze šiuos poerdvius galima užrašyti kaip
tiesinį apvalką pasirinkų bazės elementų
\begin{equation}
	\begin{split}
		A_0 &= \mathup{span}\qty(
			\mathbb 1,\,
			\vb e_2 \vb e_3,\,
			\vb e_3 \vb e_1,\,
			\vb e_1 \vb e_2
		)
		\,,
		\\
		A_0 &= \mathup{span}\qty(
			\vb e_1 ,\,
			\vb e_2 ,\,
			\vb e_3 ,\,
			\vb e_1 \vb e_2 \vb e_3
		)
		\,.
	\end{split}
\end{equation}
\begin{example}
	Tegul $v_0, v'_0 \in A_0$ ir $v_1, v'_1 \in A_1$. Turime
	\begin{equation}
		\begin{split}
			\alpha(v_0 v'_0)
			= \alpha(v_0)\alpha(v'_0)
			= v_0 v'_0
			\implies &
			v_0 v'_0 \in A_0
			\,,
			\\
			\alpha(v_0 v_1)
			= \alpha(v_0)\alpha(v_1)
			= v_0 (-v_1)
			= - v_0 v_1
			\implies &
			v_0 v_1 \in A_1
			\,,
			\\
			\alpha(v_1 v_0)
			= \alpha(v_1)\alpha(v_0)
			= (- v_1)v_0
			= - v_1 v_0
			\implies &
			v_1 v_0 \in A_1
			\,,
			\\
			\alpha(v_1 v'_1)
			= \alpha(v_1)\alpha(v'_1)
			= (- v_1)(- v'_1)
			= v_1 v'_1
			\implies &
			v_1 v'_1 \in A_0
			\,.
		\end{split}
	\end{equation}
	Taip yra tokia pati struktūra kaip $\set Z_2$ su sudetimi
	\begin{equation}
		\begin{split}
			\mathbf{lyginis} + \mathbf{lyginis} &= \mathbf{lyginis}
			\,,
			\\
			\mathbf{lyginis} + \mathbf{nelyginis} &= \mathbf{nelyginis}
			\,,
			\\
			\mathbf{nelyginis} + \mathbf{lyginis} &= \mathbf{nelyginis}
			\,,
			\\
			\mathbf{nelyginis} + \mathbf{nelyginis} &= \mathbf{lyginis}
			\,.
		\end{split}
	\end{equation}
\end{example}
Dėl tokios daugybos struktūros vektoriai edvėje $A_0$ yra vadinami \emph{lyginiais},
o vektoriai erdvėje $A_1$ yra vadinami \emph{nelyginiais}.
Pamatome, kad homomorfizmas $\alpha$ 
leidžia algebrą $\mathup{Cl}^\pm(3)$ suskaidyti 
į lyginio ir nelyginio poerdvių tiesioginę sumą $A_0 \oplus A_1$.
Sakome, kad algebrai $\mathup{Cl}^\pm(3)$ homomorfizmas $\alpha$ suteikia 
$\set Z_2$ gradavimą. Ir lyginis poerdvis yra uždaras su algebros daugyba,
todė tai yra poalgebris.

\begin{preposition}
	\label{prep:even_iso}
	Algebrų $\mathup{Cl}^+(3)$ ir $\mathup{Cl}^-(3)$
	Lyginiai poalgiabriai $A^+_0$ ir $A^-_0$ yra izomorfiškos algebros.
\end{preposition}
\begin{proof}
	Algebros $A_0^+$ ir $A_0^-$ turi bazę
	$\qty{
		\mathbb 1,\, 
		\vb e_{23},\,
		\vb e_{31},\,
		\vb e_{12}
	}$. Pastebime kad algebroje $A_0^+$
	\begin{equation}
		\vb e_{23} \vb e_{31} 
		= \vb e_2 \vb e_3 \vb e_3 \vb e_1
		= \vb e_2 \vb e_1
		= - \vb e_{12}
		\,,
	\end{equation}
	\begin{equation}
		\vb e_{31} \vb e_{23} 
		= \vb e_3 \vb e_1 \vb e_2 \vb e_3
		= \vb e_1 \vb e_3 \vb e_3 \vb e_2
		= \vb e_1 \vb e_2
		= \vb e_{12}
		\,,
	\end{equation}
	\begin{equation}
		{\vb e_{31}}^2 
		= \vb e_3 \vb e_1 \vb e_3 \vb e_1 
		= - \vb e_3 \vb e_3 \vb e_1 \vb e_1 
		= - \mathbb 1
		\,,
	\end{equation}
	\begin{equation}
		{\vb e_{23}}^2 
		= \vb e_2 \vb e_3 \vb e_2 \vb e_3 
		= - \vb e_2 \vb e_2 \vb e_3 \vb e_3 
		= - \mathbb 1
		\,,
	\end{equation}
	o algebroje $A_0^-$
	\begin{equation}
		\vb e_{23} \vb e_{31} 
		= \vb e_2 \vb e_3 \vb e_3 \vb e_1 
		= - \vb e_2 \vb e_1 
		= \vb e_1 \vb e_2 
		= \vb e_{12}
		\,,
	\end{equation}
	\begin{equation}
		\vb e_{31} \vb e_{23} 
		= \vb e_3 \vb e_1 \vb e_2 \vb e_3
		= \vb e_1 \vb e_3 \vb e_3 \vb e_2
		= - \vb e_1 \vb e_2
		= - \vb e_{12}
		\,,
	\end{equation}
	\begin{equation}
		{\vb e_{31}}^2 
		= \vb e_3 \vb e_1 \vb e_3 \vb e_1 
		= - \vb e_3 \vb e_3 \vb e_1 \vb e_1 
		= - \mathbb 1
		\,,
	\end{equation}
	\begin{equation}
		{\vb e_{23}}^2 
		= \vb e_2 \vb e_3 \vb e_2 \vb e_3 
		= - \vb e_2 \vb e_2 \vb e_3 \vb e_3 
		= - \mathbb 1
		\,.
	\end{equation}
	Abiems algebroms $\vb e_{31}$ ir $\vb e_{23}$ yra algebros generatoriai.
	Jie tenkina saryšius
	\begin{equation}
		\begin{aligned}
			\vb e_{31} \vb e_{23} &= - \vb e_{23} \vb e_{31} \\
			{\vb e_{31}}^2 &= - \mathbb 1 \\
			{\vb e_{23}}^2 &= - \mathbb 1
			\,.
		\end{aligned}
	\end{equation}
	Tai yra sąryšiai apibrėžentys Clifford'o algebrą $\mathup{Cl}^-(2)$,
	tad $\mathup{Cl}^-(2) \cong A_0^+ \cong A_0^-$.
\end{proof}
