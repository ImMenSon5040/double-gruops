...
Grupės prezentacija yra būdas apibrėžti grupę,
...
Toliau pristatysime kombinatorinės grupių teorijos pagrindus,
...
yra aibė sudaryta iš \emph{atvirkštinių raidžių}
...
Tegul tarp dviejų žodžių yra ekvivalentiškumo sąryšis
$f_1 f_2 \cdots f_n \sim f'_1 f'_2 \cdots f'_m$,
jei iš vieno žodžio galime gauti kitą
pakartotinai įterpdami raidės ir jos atvirkštinės raidės poras $xx^{-1}$ ir $x^{-1}x$, $x \in S$.
Visi žodžiai tarp kurių yra ekvivalentiškumo sąryšis sudaro ekvivalentiškumo klasę.
\begin{definition}
...
\begin{example}
	...
	\begin{itemize}
		...
		\item Egzistuoja vienetinis elementas:
			bet kurį žodį sujungus su tuščiu žodžiu $e$, gausime nepakeistą žodį.
		\item Egzistuoja atvirkštiniai elementai:
			žodžiui $f_1 f_2 \cdots f_n$ atvirkštinis yra $f_{n}^{-1} f_{n-1}^{-1} \cdots f_1^{-1}$,
			kur 
			\begin{equation}
				f_i^{-1}= 
				\begin{cases}
...
Ją sudarėme visų žodžių aibei suteikdami sąryšio $xx^{-1}=e$ struktūra ir asociatyvia uždara daugybą,
...
jos elementai vadinami \emph{sąryšio žodžiais} (ang. \emph{relators}).
...
\begin{remark}
	...
	kad nekiltų rizikos sumaišyti grupės prezentacija su Dirac'o \textquote{bra-ket} žymėjimu.
	Taip pat abėcėlė $S$ bus pateikta ne kaip aibė, o elementais.
	Sąryšio žodžiai $R$ bus pateikti kaip grupę apibrėžiantys sąryšiai
	$r' = r$, kur $r'r^{-1}\in R$.
	Sąryšiai yra patogūs, nes $F_s$ elementai $r$ ir $r'$ priklauso
...
\begin{example}
	...
	\end{equation}
	ir su grupės elementais galime atlikti skaičiavimus kaip kad
	\begin{equation}
		abab = abba = aea = a^2
		\,.
	\end{equation}
...
\begin{remark}
	Svarbu paminėti, kad grupės prezentacija nėra unikali.
	\begin{itemize}
		...
		\item Galime pridėti generatorių $d$ prie abėcėlės $S$
			...
			kur $w$ yra bet koks žodis iš pirminės abėcėlės $S$ raidžių,
